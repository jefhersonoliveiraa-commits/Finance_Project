#!/usr/bin/env bash
# aplicar-import-encoding.sh
# Corrige a importacao para bancos brasileiros:
# - Autodeteccao de encoding: tenta UTF-8, cai para ISO-8859-1 se
#   necessario (arquivos CSV do Nubank, C6 e Bradesco sao latin1)
# - Amplifica os padroes de deteccao de colunas (nomes exatos do
#   Nubank, C6, Bradesco: "Lançamento", "Valor (R$)", etc.)
set -euo pipefail
git rev-parse --is-inside-work-tree >/dev/null 2>&1 || { echo "ERRO: rode na raiz do repo."; exit 1; }
cd "$(git rev-parse --show-toplevel)"
STAMP="$(date +%Y%m%d-%H%M%S)"
TAG="backup/pre-import-fix-${STAMP}"
git tag "$TAG"
echo "==> Backup: $TAG"
...
function parseBRLAmount(raw: string): number {
  ...
  if (s.includes('.') && s.includes(',')) {
    ...
    if (parts.length === 2 && parts[1].length <= 2) {
      ...
      // Thousands comma: 1,234,567 (remove commas)
      s = s.replace(/,/g, '')
    }
  }
...
export function parseCSV(content: string, mapping: ColumnMapping, delimiter: string = ','): ParsedTransaction[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (typeIdx >= 0 && values[typeIdx]) {
      ...
      type = typeVal.includes('credito') || typeVal.includes('credit') || typeVal.includes('entrada') ? 'credit' : 'debit'
    } else {
      // Use sign from the raw string before stripping it
      const rawSigned = values[amountIdx].replace(/[R$\s]/g, '')
      type = rawSigned.startsWith('-') ? 'debit' : 'credit'
    }
...
git add -A
git commit -m "fix: importacao com autodeteccao de encoding (UTF-8/latin1) para Nubank, C6 e Bradesco"
